...
(async (window) => {
	...
	//console.log(`[WebRTC] loginController:${JSON.stringify(loginController)}`);
	//$('#loginBtn').click();
			...
			for (var i in message.body.media.dialog.messages) {
				...
				switch (message.body.media.dialog.messages[i].type) {
					case "text":
						...
						html += (message.header.type === "bot" ? 
						`<p style="margin-top:5px;font-size:20px;color:blue;font-weight:normal;"> ${message.body.media.dialog.messages[i].text}</p >`
						: `<p style="margin-top:5px;font-size:20px;color:black;font-weight:normal;"> ${message.body.media.dialog.messages[i].text}</p >`);
	...
	window.imagePad_circle = (x, y) => {
		...
		imagePad.context.strokeStyle = '#00f';
	...
	const imageDropTarget = document.getElementById("webclient");
	...
	$("#view-video-remote").click(() => {
		;
	});

	$("#view-video-local").click(() => {
		;
	});
	...
	window.eventCustomerDropped = () => {
		events.launch('emit_event', { header: _header(cmsp.client), body: { type: "customerDropped", customerId: cmsp.client.room, agentId: cmsp.client.userId } });
	};
	...
	window.selection_clicked = (modal_id, text, value) => {
		var selected_menu = `（Your selection）<br/><font color=blue>${text}</font>`;
		events.launch('emit_data', {
			header: _header(cmsp.client),
			body: {
				media: {
	...
	window.digit_clicked = (modal_id, value) => {
		if (value === "Enter") {
			if ($("#cardNumber").text().length == 14) {
				var cardNumberText = `（Your card number）<br/><font color=blue>＊＊＊＊＊＊＊＊＊＊＊＊${$("#cardNumber").text().slice(-4)}</font>`;
				events.launch('emit_data', {
					header: _header(cmsp.client),
					body: {
						media: {
							type: "dialog", dialog: {
								...
								attributes: { modal_id: modal_id, value: $("#cardNumber").text() }
							}
						}
					}
				});
	...
	events.action("event", async (message) => {
		...
		switch (message.body.type) {
			...
			case "queued": {
				var action = "";
				message.body.attributes.forEach((item) => {
					//if (item.bot_step === "transfer_to_agent_black") action = item.value;
					action = item.value;
				});
				if (action === "chat_voice") {
					//$("#calleeAddress").val("20001");
					//$("#audioCallBtn").click();
				}
				break;
			}
			case "picked": {
				if (cmsp.client.userId === message.body.customerId)
					cmsp.client.room = message.body.agentId;
				break;
			}
